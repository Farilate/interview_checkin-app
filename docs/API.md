...
ID 使用十进制正整数字符串。时间戳为 ISO 8601 UTC，例如 `2026-09-17T02:00:00.000Z`；业务日期为 `YYYY-MM-DD`。
...
- 以下登录、习惯和打卡章节仍是业务接口契约；阶段 3 不新增这些业务端点。验证用 `/probe/*` 接口只存在于测试目录，不打包进生产应用。
