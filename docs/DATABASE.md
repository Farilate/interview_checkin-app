...
业务日期由 Clock 时区决定，UTC 打卡时间截断到毫秒写入 DATETIME(3)。每次请求只读取一次 Instant 并截断到毫秒，业务日期与 UTC 时间都从该瞬间派生。今日状态和连续天数查询已实现，分别直接读取今日记录或截至当天的倒序日期。已接入 today/streak 缓存，首次与重复打卡成功均尝试失效缓存。真实 MySQL Mapper 和 Service 10 线程并发测试通过，真实 HTTP 并发验收仍待执行。
...
缓存校验、批量删除及 TTL 边界已修复。2026-09-20，用户已确认真实 Memurai 的 TTL、Key 内容、写后失效和自然过期验收通过。真实网络故障及并发回填窗口验收尚未确认；上述通过结果不代表缓存强一致。
