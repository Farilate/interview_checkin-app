...
业务日期由 Clock 时区决定，UTC 打卡时间截断到毫秒写入 DATETIME(3)。每次请求只读取一次 Instant 并截断到毫秒，业务日期与 UTC 时间都从该瞬间派生。今日状态和连续天数查询已实现，分别直接读取今日记录或截至当天的倒序日期。已接入 today/streak 缓存，首次与重复打卡成功均尝试失效缓存。真实 MySQL Mapper 和 Service 10 线程并发测试通过，真实 HTTP 鉴权并发验收已完成。
...
| 今日状态 `checkin:v1:today:{uid}:{hid}:{D}` | 字符串 "1" / "0"，对应 checkedIn=true/false | 默认 30 秒，且不得跨到下一个业务日继续使用 | 查询 Miss 后从 MySQL 回源并写入 | 插入成功、顺序重复或冲突回查成功后尝试删除；读取不续期 | 查询 MySQL 恢复真实状态 |
| 连续天数 `checkin:v1:streak:{uid}:{hid}:{D}` | 十进制整数字符串，对应 streak；0 也缓存 | 默认 30 秒，且不得跨到下一个业务日继续使用 | 查询 Miss 后从 MySQL 计算并写入 | 打卡事务提交后删除；读取不续期 | 从 MySQL 倒序读取并重算 |

登录态放 Redis 是为了服务端认证和集中失效；今日状态和连续天数放 Redis 是为了减少重复查询。所有真实业务记录始终保存在 MySQL。

登录态、today 和 streak 三类缓存均已接入。业务缓存配置 app.cache.ttl-seconds 来自 APP_CACHE_TTL_SECONDS，默认 30 秒；实际 TTL 为配置时长与距键所属日期下一零点时长的较小值。TTL 配置非正数时 Bean 创建失败；过期日期或实际 TTL 不足 1ms 时跳过回填。会话摘要为原始 Token 的 SHA-256 小写十六进制字符串；不保存原始 Token、密码、issuedAt 或 expiresAt。有效期由 Redis TTL 管理，配置属性为 `app.session.ttl-seconds`，当前 YAML 使用 `SESSION_TTL_SECONDS` 注入。
...
## 7. MySQL / Redis 业务缓存一致性策略（当前实现与限制）
...
3. 缓存删除失败时记录脱敏日志；当前没有重试；但不得为了删缓存回滚已经成功的 MySQL 事务。
...
- Cache-Aside 不保证强一致：并发旧查询可在删除之后填入旧值，或删除失败后保留旧值；当前默认 TTL 30 秒，用短 TTL 最终收敛。
- getToday 严格只接受 "0"/"1"，getStreak 只接受非负整数；损坏值按 Miss 回源，putStreak 跳过负数写入。
- evict 一次 delete(Collection) 提交 today/streak 两个键；删除异常记录固定警告并忽略，不回滚已成功的 MySQL 操作。
- TTL 配置必须 >0，否则 Bean 创建失败。实际 TTL 取配置值与距午夜时长的较小值，低于 1ms 不回填，避免驱动取整歧义。
- Service 未使用事务提交后回调；当前无外层事务时先完成数据库写入再删缓存。若未来增加外层事务，需重新验证提交与失效顺序。

缓存校验、批量删除及 TTL 边界已修复。2026-09-20，真实 Memurai 的 TTL、Key 内容、写后失效和自然过期验收通过。阶段 1–8 的全部真实验收已完成，包含真实网络故障及并发缓存边界；通过验收不改变短 TTL 最终一致性设计，不代表缓存强一致。
